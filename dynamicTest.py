try:
    ...
    from runnerDynamic import Runner
...
parser.add_argument('-d','--testData', help='Number of test data', required=False)
        ...
        if args.urlNum == 'all':
            ...
            for i, url in enumerate(my_dict['BaseUrl']):
                ...
                try:
                    ...
                    log.logger.warning("Verifying all data sets for test case %d" % (i+1))
                    try:
                        for d in range(int(args.testData)):
            ...
            try:
                ...
                log.logger.warning("Verifying all data sets for test case %d" % (a+1))
                try:
                    for d in range(int(args.testData)):
                        resp = Runner.runTest(baseurl=my_dict['BaseUrl'][a], path=my_dict['Path'][a], params=my_dict['Params'][a], 
                                      ...
                                      files=None, auth=my_dict['Auth'][a], method=my_dict['Method'][a], dataCount=d)
                    ...
                    log.logger.error("Test case %d passed with all test data" % (a+1))
                    log.logger.debug("Test case \"%s\" passed" % my_dict['Description'][a])
                    print ("Test case %d passed with all test data" % (a+1))
            except Exception as e:
                print ("Test condition failed", e)
    except Exception as e:
